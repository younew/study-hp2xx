 ...
 **/
{
	...
	pg->quiet = FALSE;
	pg->extraclip = 0;
	pg->maxpensize = 0.025;	/*0.025 mm zzg //0.1  1/10 mm              */
 ...
 ** a temporary file.
 ** 这个函数打开一个HPGL文件，扫描并解析hpgl指令，然后将基本的move、draw指令写入一个临时文件
 ...
 ** the picture buffer.
 ** 函数返回后HPGL文件将关闭，但是临时文件仍然打开。你可以多吃使用。但最终一定要关闭。
 ** 处理阶段后，调用该函数是无效的，比如图片缓存区。？？？
 **/
extern FILE* td_debug;
int HPGL_to_TMP(GEN_PAR * pg, IN_PAR * pi)
{
	printf("\r\n-----------HPGL_to_TMP--------------\r\n");
	if (td_debug == NULL)
		if ((td_debug = fopen("D:\\work\\ex\\hp2xx-3.4.4\\makes\\visualc6.00\\Debug\\temp.txt", "w")) == NULL)
			printf("open td_debug file filed\r\n");
  /**
...
int TMP_to_VEC(const GEN_PAR * pg, const OUT_PAR * po)
{
	if (pg->td == NULL)
		return ERROR;
	rewind(pg->td);		/* 使文件fp的位置指针指向文件开始 Rewind temp file for re-reading      */
	if (n_commands < 0)
		return 0;
