...
void HPGL_Pt_to_tmpfile(const HPGL_Pt * pf)
{
	if (record_off)		/* Wrong page!  */
		return;
	printf("xy=%0.4f,%0.4f\r\n", pf->x, pf->y);
	fwrite((VOID *) pf, sizeof(*pf), 1, td_debug) ;
...
int read_PE_flags(GEN_PAR * pg, int c, FILE * hd, PE_flags * fl)
{
	...
	case ':':
		...
		if (pen == 0 && pg->mapzero > -1)
			pen = pg->mapzero;
		if (old_pen != pen) {
			printf("temp pen:%d",SET_PEN);
			printf("temp pen:%d",pen);
			fputc(SET_PEN, td_debug);
			fputc(pen, td_debug);
